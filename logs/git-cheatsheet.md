...
新しい環境では `checkout` より、役割が明確な `switch` と `restore` を優先すると覚えやすいです。
...
Notionメモに含まれていた `git reset --hard` は危険度が高いため、用途と注意点を明記しました。公開記事では、個人名やチーム固有の略称は載せていません。
